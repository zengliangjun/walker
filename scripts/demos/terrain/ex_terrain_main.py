...
from isaaclabex.terrains import ext_importer, ext_importer_cfg
...
from scripts.demos.terrain.parkour_test import PARKOUR_TERRAINS_CFG 
...
def design_scene() -> tuple[dict, torch.Tensor]:
    ...
    terrain_gen_cfg = PARKOUR_TERRAINS_CFG.replace(curriculum=args_cli.use_curriculum, color_scheme=args_cli.color_scheme)

    '''
    for _, sub_terrain_cfg in terrain_gen_cfg.sub_terrains.items():
    ...
    if args_cli.show_flat_patches:
        for sub_terrain_name, sub_terrain_cfg in terrain_gen_cfg.sub_terrains.items():
            sub_terrain_cfg.flat_patch_sampling = {
                sub_terrain_name: FlatPatchSamplingCfg(num_patches=10, patch_radius=0.3, max_height_diff=0.15)
            }

    # Handler for terrains importing
    terrain_importer_cfg = ext_importer_cfg.TerrainImporterCfg(
        num_envs=128, #2048,
        env_spacing=3.0,
    ...
    terrain_importer = ext_importer.TerrainImporter(terrain_importer_cfg)
    ...
    if args_cli.show_flat_patches:
        ...
        for name in terrain_importer.flat_patches:
            vis_cfg.markers[name] = sim_utils.CylinderCfg(
                radius=0.2,  # note: manually set to the patch radius for visualization
                height=3.15,
